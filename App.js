...
// Transit meanings — what each planet in a sign means right now
const TRANSIT_MEANINGS = {
  sun: 'The Sun illuminates themes of identity and self-expression in this area of life.',
  moon: 'The Moon stirs emotions and intuition, bringing feelings to the surface.',
  mercury: 'Mercury sharpens thinking and communication around these themes.',
  venus: 'Venus softens and beautifies, bringing harmony and attraction here.',
  mars: 'Mars energizes and activates, pushing you to take bold action.',
  jupiter: 'Jupiter expands opportunities and brings growth and optimism.',
  saturn: 'Saturn asks for discipline and commitment — the work pays off.',
  uranus: 'Uranus shakes things up with unexpected changes and breakthroughs.',
  neptune: 'Neptune dissolves boundaries, heightening intuition and creativity.',
  pluto: 'Pluto drives deep transformation — something is being reborn.',
};
...
function DailyReadingScreen({ planets }) {
  ...
  return (
    <ScrollView style={st.screen} contentContainerStyle={{ paddingHorizontal: 24, paddingBottom: 40 }}
      ...
      {(liveTransits || []).map(t => (
        <GlassCard key={t.key} style={{ marginBottom: 10 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center' }}>
            <ZodiacBadge sign={t.sign} size={42} />
            <View style={{ marginLeft: 14, flex: 1 }}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                <Text style={{ color: C.cream, fontSize: 15, fontFamily: F.bodyMed }}>{t.planet}</Text>
                {t.retrograde && <View style={{ backgroundColor: 'rgba(212,160,160,0.15)', paddingHorizontal: 6, paddingVertical: 2, borderRadius: 6 }}><Text style={{ color: C.blush, fontSize: 10, fontFamily: F.bodySemi }}>Retrograde</Text></View>}
              </View>
              <Text style={{ color: C.gold, fontSize: 13, fontFamily: F.body, marginTop: 2 }}>{t.sign} {t.degree}° · {t.element}</Text>
            </View>
            <ZodiacIcon sign={t.sign} size={18} color={C.muted} />
          </View>
          {/* Transit meaning */}
          <Text style={{ color: C.creamDim, fontSize: 13, lineHeight: 20, fontFamily: F.displayItalic, marginTop: 10, paddingTop: 10, borderTopWidth: StyleSheet.hairlineWidth, borderTopColor: C.border }}>{t.meaning}</Text>
        </GlassCard>
      ))}
              ...
              <View style={{ marginTop: 14, paddingTop: 14, borderTopWidth: StyleSheet.hairlineWidth, borderTopColor: C.border }}>
                {data && (
                  <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: 10 }}>
                    <View style={st.tagPill}><Text style={st.tagText}>House {data.house}</Text></View>
                ...
                <Text style={{ color: C.creamDim, fontSize: 14, lineHeight: 22, fontFamily: F.displayItalic }}>
                  {p.meaning ? p.meaning.short : `Your ${p.name.toLowerCase()} placement reveals how this energy uniquely shapes your life.`}
                </Text>
              </View>
            )}
          </GlassCard>
